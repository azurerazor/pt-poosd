...
import MobileGame, { MOBILE_ROLES } from "@api/models/MobileGame.js";
...
router.post("/put", async (req: Request, res: Response) => {
  ...
  if (goodWin !== true && goodWin !== false) {
    res.status(400).json({ message: "Invalid goodWin value" });
    return;
  }
  ...
  if (missionOutcomes.length != 5) {
    res.status(400).json({ message: "Invalid mission outcomes" });
    return;
  }
  ...
  if (existingGame) {
    res.status(403).json({ message: "Game already exists" });
    return;
  }
